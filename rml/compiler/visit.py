...
@Pattern
def visit(asdl, ctx, cr):
    return type(asdl)


def _make_overload(pat_func):
...
globals()['overload'] = _make_overload(visit)


@overload
def visit(asdl: basic.ASDL, ctx: Context, cr: Compiler):
    pass
...
def visit(asdl: expr.ArrayLiteral, ctx: Context, cr: Compiler):
    ...
    cr.add_inst(bc['BUILD_TUPLE'], len(asdl.elements), lineno)
